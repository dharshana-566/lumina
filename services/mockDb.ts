...
import { User, Product, Order, UserRole, OrderStatus, Address, PaymentMethod } from '../types';
...
const generateProducts = (categories: string[]): Product[] => {
  const curatedData: Record<string, { 
    name: string, 
    material: string, 
    color: string, 
    finish: string, 
...
const INITIAL_DATA: DB = {
  users: [
    { 
      id: '1', name: 'Admin User', email: 'admin@lumina.com', role: UserRole.ADMIN, password: 'password123',
    ...
    { 
      id: '2', name: 'John Doe', email: 'john@example.com', role: UserRole.USER, password: 'password123',
...
class MockDB {
  ...
  addUser(user: User) { this.data.users.push({ ...user, addresses: [], paymentMethods: [] }); this.save(); }
  ...
  addProduct(product: Product) { this.data.products.push(product); this.save(); }
  updateProduct(id: string, updates: Partial<Product>) {
    this.data.users = this.data.users; // No-op to trigger save if needed elsewhere
  ...
  addOrder(order: Order) { this.data.orders.push(order); this.save(); }
